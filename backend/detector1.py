...
# # ----------------------------------------------------------------------
# # Common name list (keep your existing list – truncated for brevity)
# # ----------------------------------------------------------------------
# COMMON_NAMES = { 
#      "ram", "rama", "shyam", "ravi", "kumar", "raj", "rani", "amit", "sunil",
#     "vijay", "ajay", "suresh", "mahesh", "rohit", "sharma", "ganesha", "rahul",
...
# # ----------------------------------------------------------------------
# # Single‑word and multi‑word organization lists
# # ----------------------------------------------------------------------
# SINGLE_WORD_ORGS = { 
#        "google", "microsoft", "apple", "amazon", "facebook", "meta", "netflix",
#     "spotify", "twitter", "x", "linkedin", "whatsapp", "youtube", "instagram",
...
# # ----------------------------------------------------------------------
# # Pre‑compiled regex patterns (strict, no newline matching)
# # ----------------------------------------------------------------------
# _EMAIL_RE = re.compile(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b')

# # Phone: now requires word boundaries and avoids matching inside emails
# _PHONE_RE = re.compile(
...
# # Generic ID: 9+ consecutive digits (no hyphens)
# _ID_RE = re.compile(r'\b\d{9,}\b')

# # Address: requires number, at least two words, and a common suffix
# _ADDRESS_RE = re.compile(
#     r'\b\d{1,5}[ \t]+[A-Za-z]+(?:[ \t]+[A-Za-z]+){1,4}'
#     r'(?:[ \t]+(?:road|rd|street|st|avenue|ave|lane|ln|drive|dr|court|ct|plaza|way|apt|bldg|suite|ste))?\b',
#     re.IGNORECASE
# )
...
# # All‑caps orgs (≥3 letters)
# _ALLCAPS_ORG_RE = re.compile(r'\b[A-Z]{3,}\b')

# # Fallback name detector is DISABLED (commented out)
# # _NAMES_FALLBACK_RE = re.compile(r'\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)+\b')
...
#     matches = []
#     for m in _ADDRESS_RE.finditer(text):
#         # Avoid matching overly long strings (like entire paragraphs)
#         if 10 < len(m.group()) < 100:
#             matches.append({"label": "ADDRESS", "text": m.group(), "start": m.start(), "end": m.end()})
#     return matches
...
#         entities.extend(_detect_multi_word_orgs(text))
#         entities.extend(_detect_allcaps_orgs(text))
#         # Fallback name detector disabled
#     except Exception as e:
#         logger.exception("Regex detection failed")
#     return entities
...
#     for next_ent in sorted_ents[1:]:
#         if next_ent['start'] < current['end']:
#             # Overlap: keep the longer span, but if equal length, keep the higher priority
#             if (next_ent['end'] - next_ent['start']) > (current['end'] - current['start']):
#                 current = next_ent
#         else:
#             resolved.append(current)
...
# # ----------------------------------------------------------------------
# # Priority for overlap resolution (higher number = more important)
# # ----------------------------------------------------------------------
# PRIORITY_MAP = {
#     "EMAIL": 5,
#     "PHONE": 4,
...
# # ----------------------------------------------------------------------
# # Common name list (case‑insensitive) – keep your full list
# # ----------------------------------------------------------------------
# COMMON_NAMES = {
#     "ram", "rama", "shyam", "ravi", "kumar", "raj", "rani", "amit", "sunil",
#     "vijay", "ajay", "suresh", "mahesh", "rohit", "sharma", "ganesha", "rahul",
...
# # ----------------------------------------------------------------------
# # Pre‑compiled regex patterns (strict, with length limits)
# # ----------------------------------------------------------------------
# # Email
# _EMAIL_RE = re.compile(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b')

# # Phone – now requires word boundaries and at least 10 digits
# _PHONE_RE = re.compile(
#     r'\b(?:\+\d{1,3}[ \t-]?)?\(?\d{3}\)?[ \t-]?\d{3}[ \t-]?\d{4}\b',
#     re.VERBOSE
# )
...
# # Generic ID – at least 9 digits, no separators
# _ID_RE = re.compile(r'\b\d{9,}\b')

# # Address – requires number, at least two words, and a common suffix, max 80 chars
# _ADDRESS_RE = re.compile(
#     r'\b\d{1,5}[ \t]+[A-Za-z]+(?:[ \t]+[A-Za-z]+){1,4}'
#     r'(?:[ \t]+(?:road|rd|street|st|avenue|ave|lane|ln|drive|dr|court|ct|plaza|way|apt|bldg|suite|ste))?\b',
#     re.IGNORECASE
...
#     matches = []
#     for m in _PHONE_RE.finditer(text):
#         # Ensure the match doesn't contain letters (shouldn't with \b, but extra safety)
#         if not re.search(r'[a-zA-Z]', m.group()):
#             matches.append({"label": "PHONE", "text": m.group(), "start": m.start(), "end": m.end()})
#     return matches
...
#     matches = []
#     for m in _ADDRESS_RE.finditer(text):
#         # Avoid matching very long or very short strings
#         if 10 < len(m.group()) < 80:
#             # Ensure it contains a number and a street suffix (rough check)
#             if re.search(r'\b\d+\b', m.group()) and re.search(r'\b(?:road|rd|street|st|avenue|ave|lane|ln|drive|dr|court|ct|plaza|way|apt|bldg|suite|ste)\b', m.group(), re.IGNORECASE):
#                 matches.append({"label": "ADDRESS", "text": m.group(), "start": m.start(), "end": m.end()})
#     return matches
...
#     # Sort by start, then priority descending, then length descending
#     sorted_ents = sorted(
#         entities,
#         key=lambda e: (e['start'], -PRIORITY_MAP.get(e['label'], 0), - (e['end'] - e['start']))
#     )
...
#     for next_ent in sorted_ents[1:]:
#         if next_ent['start'] < current['end']:
#             # Overlap: keep the higher priority, or if same priority keep the longer
#             if (PRIORITY_MAP.get(next_ent['label'], 0) > PRIORITY_MAP.get(current['label'], 0) or
#                 (PRIORITY_MAP.get(next_ent['label'], 0) == PRIORITY_MAP.get(current['label'], 0) and
#                  (next_ent['end'] - next_ent['start']) > (current['end'] - current['start']))):
#                 current = next_ent
#         else:
#             resolved.append(current)
#             current = next_ent
