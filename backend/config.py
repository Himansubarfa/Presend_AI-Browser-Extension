...
# Changes vs day 26:
#   MAX_ADDRESS_LENGTH  raised 100 → 200 (full international addresses need the headroom)
#   ENABLE_POSTCODE     added (global postcode detector; distinct from ENABLE_PINCODE)
...
class Config:
    ...
    class Detectors:
        ...
        # ── Global postcode (NEW) ─────────────────────────────────────────────
        # Covers UK, US ZIP, Canada, Germany, France, Australia, Netherlands,
        # Brazil, Japan, South Korea, Singapore in addition to India 6-digit.
        # Set ENABLE_POSTCODE=false to disable (India PINCODE has its own flag).
        ENABLE_POSTCODE = os.getenv('ENABLE_POSTCODE', 'true').lower() == 'true'
        ...
        # ── Address length bounds ─────────────────────────────────────────────
        # MIN raised slightly; MAX raised from 100 → 200 for international addresses
        # e.g. "Apartment 4B, 123 Koramangala 5th Block, Bengaluru, Karnataka 560095"
        # is 68 chars; a full US address with unit + city + state + ZIP is ~80-120.
        MIN_ADDRESS_LENGTH = int(os.getenv('MIN_ADDRESS_LENGTH', 10))
        MAX_ADDRESS_LENGTH = int(os.getenv('MAX_ADDRESS_LENGTH', 200))