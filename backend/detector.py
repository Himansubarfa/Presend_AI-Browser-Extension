...
#         })
#     return result
# def _detect_indian_streets(text: str) -> List[Dict[str, Any]]:
#     suffixes = r'(?:nagar|ganj|chowk|marg|road|street|colony|sector|vihar|'
#     suffixes += r'enclave|layout|puram|wadi|katte|halli|palya|bazaar|'
#     suffixes += r'lane|drive|avenue|gardens|grove|plaza|court|'
#     suffixes += r'rd|ave|blvd|cres|hwy|apartment|building|apt|bldg|ste)'

#     pattern = re.compile(
#         r'\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?\s*,?\s*' + suffixes + r'\b',
#         re.IGNORECASE
#     )
#     result = []
#     for m in pattern.finditer(text):
#         span = m.group().strip().rstrip(',')
#         if len(span) < 6:
#             continue
#         result.append({
#             "label": "ADDRESS_INDIAN",          # temporary high‑priority label
#             "text": span,
#             "start": m.start(),
#             "end": m.end(),
#         })
...
#         if d.ENABLE_SALARY:          entities.extend(_detect_salary(text))
#         if d.ENABLE_ADDRESS:         entities.extend(_detect_addresses(text))
#         if d.ENABLE_ADDRESS:         entities.extend(_detect_indian_streets(text))
#         # ── Name / location / org patterns (lower precision) ───────────────
#         if d.ENABLE_COMMON_NAMES:    entities.extend(_detect_common_names(text))
#         if d.ENABLE_INDIAN_CITIES:   entities.extend(_detect_indian_cities(text))
#         if d.ENABLE_SINGLE_ORGS:     entities.extend(_detect_single_word_orgs(text))
...
#     """Add ADDRESS for whole lines that contain a digit, an Indian city,
#     and a street suffix. Removes any overlapping weaker entities."""
...
#         has_city = bool(re.search(city_pattern, stripped))
#         has_suffix = bool(re.search(suffixes, stripped, re.IGNORECASE))

#         if has_digit and has_city and has_suffix:
#             start = text.index(stripped, pos)
#             end = start + len(stripped)

#             # Remove any existing entities that overlap with this address line
#             entities = [e for e in entities if not (e['start'] < end and e['end'] > start)]
#             entities.append({'label': 'ADDRESS', 'start': start, 'end': end, 'text': stripped})
...
_ADDRESS_RE = re.compile(
    ...
    r'\b\d{1,5}[A-Za-z]?(?:[\-/]\d{1,4})?\s+[A-Za-z]{2,}(?:\s+[A-Za-z]{2,}){0,3}\s+' + _ADDRESS_SUFFIX_GLOBAL + r'\b'
    r'|'
    # PAT2: Suffix-first — MUST be followed by a number (house/plot/flat no.)
    r'\b[A-Za-z]{2,}(?:\s+[A-Za-z]{2,}){0,4}\s+' + _ADDRESS_SUFFIX_GLOBAL + r'\b\s+\d{1,5}'
    r'|'
    # PAT3: Flat/Plot/House/Door/No. prefix
    r'\b(?:flat|plot|house|door|apartment|no\.?)\s*(?:\d{1,4}[A-Za-z]?|[A-Za-z]\d{1,4})'
...
def _detect_indian_streets(text: str) -> List[Dict[str, Any]]:
    """Catch 'Word Suffix' Indian street patterns, but skip known person names."""
    suffixes = r'(?:nagar|ganj|chowk|marg|road|street|colony|sector|vihar|'
    suffixes += r'enclave|layout|puram|wadi|katte|halli|palya|bazaar|'
    suffixes += r'lane|drive|avenue|gardens|grove|plaza|court|'
    suffixes += r'rd|ave|blvd|cres|hwy|apartment|building|apt|bldg|ste)'
    ...
    result = []
    for m in pattern.finditer(text):
        word_part = m.group(1).strip()          # e.g. "Parekh" or "Rahul Kumar"
        span = m.group().strip().rstrip(',')

        if len(span) < 6:
            continue

        # Skip if the word before the suffix is a known person name
        first_word = word_part.split()[0].lower()
        if first_word in COMMON_NAMES:
            continue

        # Require a digit on the same line
        line_start = text.rfind('\n', 0, m.start()) + 1
        line_end = text.find('\n', m.end())
        if line_end == -1:
            line_end = len(text)
        if not re.search(r'\d', text[line_start:line_end]):
            continue
...
def _validate_entity(ent: Dict[str, Any], text: str) -> bool:
    ...
    if label == "PERSON":
        ...
        if len(span.strip()) <= 1:
            return False
        words = span.split()
        # Reject single-word spans not in the known-names list or in the blacklist
        if len(words) == 1:
            low = span.lower()
            if low not in COMMON_NAMES or low in PERSON_FALSE_POSITIVES:
                return False
            ...
            if span.isupper() and len(span) <= 3:
                return False
        # Reject multi-word spans where ANY word is a known false-positive label
        if len(words) >= 2:
            lows = [w.lower().rstrip(':') for w in words]
            if any(w in PERSON_FALSE_POSITIVES for w in lows):
                return False
            # Reject if the span matches a structural label pattern (e.g. "Full Name", "Primary Contact")
            if _STRUCTURAL_LABEL_RE.match(span.strip()):
                return False
    return True
